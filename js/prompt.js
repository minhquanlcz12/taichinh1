...
const PromptModule = {
    ...
    render: () => {
        ...
        if (PromptModule.data.prompts.length === 0) {
            html += `<div style="grid-column: 1 / -1; text-align: center; color: var(--text-secondary); padding: 40px; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px dashed var(--border-color);">Chưa có Prompt nào. Hãy tạo một cái mới!</div>`;
        } else {
                ...
                html += `
                    <div class="glass-card" style="display: flex; flex-direction: column; padding: 20px; transition: transform 0.2s; border-top: 3px solid var(--primary);">
                        <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                            <h4 style="color: #fff; margin: 0; font-size: 16px; line-height: 1.3;">${p.title}</h4>
                            <div style="display: flex; gap: 8px; flex-shrink: 0;">
                                <button class="btn btn-outline btn-sm" onclick="PromptModule.showModal('${p.id}')" style="padding: 4px 8px; height: max-content;" title="Sửa">
                                    <i class="fa-solid fa-pen"></i>
                                </button>
                                <button class="btn btn-outline btn-sm" onclick="PromptModule.deletePrompt('${p.id}')" style="padding: 4px 8px; height: max-content; color: var(--danger); border-color: rgba(248, 113, 113, 0.3);" title="Xóa">
                        ...
                        <button class="btn btn-success" onclick="PromptModule.copyToClipboard('${p.id}', this)" style="width: 100%; font-weight: bold; background: linear-gradient(135deg, #10b981, #059669); border: none; font-size: 13px;">
                            <i class="fa-solid fa-copy" style="margin-right: 8px;"></i>COPY PROMPT (SAO CHÉP)
                        </button>
                    </div>
                `;
            });
    ...
    savePrompt: async () => {
        ...
        if (id) {
            ...
            if (idx !== -1) {
                PromptModule.data.prompts[idx] = { id, title, desc, content };
            }
        } else {
            // Thêm mới
