...
$hairStartStr = "        // 5. Hair Layer (V5 PREMIUM - 10 STYLES)"
$gearStartStr = "        // 8. Clothing & Vietnamese Theme (V5)"

$startIndex = $content.IndexOf($hairStartStr)
$endIndex = $content.IndexOf($gearStartStr)

if ($startIndex -eq -1 -or $endIndex -eq -1) {
    Write-Error "Could not find delimiters in chibi.js"
    exit 1
}
...
$newContent = $content.Substring(0, $startIndex) + $v7CleanBlock + $content.Substring($endIndex)

[System.IO.File]::WriteAllText($path, $newContent, [System.Text.Encoding]::UTF8)
Write-Output "Successfully repaired chibi.js and applied V7 Phase A & B via PowerShell."
