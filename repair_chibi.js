const fs = require('fs');
const path = 'C:\\Users\\Admin\\Desktop\\taichinh1\\js\\chibi.js';

let content = fs.readFileSync(path, 'utf8');

// Use a regex to find the entire block from hair styles to mouth styles and replace it with clean code
// We'll target the start of the hair styles and the end of the mouth styles.

const hairStartStr = "        // 5. Hair Layer (V5 PREMIUM - 10 STYLES)";
const gearStartStr = "        // 8. Clothing & Vietnamese Theme (V5)"; // This is usually after the corrupted part

const startIndex = content.indexOf(hairStartStr);
const endIndex = content.indexOf(gearStartStr);

if (startIndex === -1 || endIndex === -1) {
    console.error("Could not find delimiters", { startIndex, endIndex });
    process.exit(1);
}

const v7CleanBlock = `        // 5. Hair Layer (V7 MEGA OVERHAUL - 10 STYLES)
        let hairHtml = '', backHairHtml = '';
        const hStyle = c.hairStyle;
        const hc = c.hairColor;
        const darkHair = this.adjustColor(hc, -30);

        if (hStyle === 1) { // 🌪️ Super Saiyan Spike
            hairHtml = \`
        ...
        } else if (hStyle === 6) { // 🌹 Elegant Long (Nữ)
        ...
        if (ey === 0) { // Large Shine Anime (V7)
            ...
            \`;
        } else if (ey === 4) { // ^_^ Happy Closed (V7)
            eyesHtml = \`
            ...
            \`;
        } else if (ey === 8) { // 👁️ Ultimate Mystic
            eyesHtml = \`
                <g class="chibi-blink">
            ...
            \`;
        } else if (ey === 11) { // ✨ Super Sparkle (V7)
            eyesHtml = \`
...
const newContent = content.substring(0, startIndex) + v7CleanBlock + content.substring(endIndex);

fs.writeFileSync(path, newContent, 'utf8');
console.log("Successfully repaired chibi.js and applied V7 Phase A & B.");
